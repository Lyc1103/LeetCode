...
void DFSFindSubsets(int *nums, int numsSize, int pos, int **ret, int *returnSize, int **returnColumnSizes, Stack *sta)
{
	...
	for (int i = pos + 1; i < numsSize; i++)
	{
		...
		DFSFindSubsets(nums, numsSize, i, ret, returnSize, returnColumnSizes, sta);
		popFromStack(sta);
	}
	return;
}
...
int **subsets(int *nums, int numsSize, int *returnSize, int **returnColumnSizes)
{
	...
	*returnColumnSizes = (int *)malloc(malloc_size * sizeof(int));
	...
	DFSFindSubsets(nums, numsSize, -1, ret, returnSize, returnColumnSizes, sta);
	return ret;
}
...
{
	FILE *pfile;
	if ((pfile = fopen("0078.txt", "r")) == NULL)
	{
	...
	int **ret = subsets(nums, numsSize, &returnSize, &returnColumnSize);
