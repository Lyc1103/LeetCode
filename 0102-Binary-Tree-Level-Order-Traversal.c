...
{
	*returnSize = 0;
	int *rev_returnColumnSizes = (int *)calloc(1001, sizeof(int));
	int **rev_ret = (int **)calloc(1001, sizeof(int *));
	...
	{
		rev_ret[*returnSize] = (int *)calloc(T.top + 1, sizeof(int));
		rev_returnColumnSizes[*returnSize] = T.top + 1;
		...
		while (T.top != -1)
		{
			...
			ptr = Pop(&T);
			rev_ret[*returnSize][pos++] = ptr->val;
	...
	*returnColumnSizes = (int *)calloc(1001, sizeof(int));
	...
	{
		ret[i] = rev_ret[*returnSize - 1 - i];
		(*returnColumnSizes)[i] = rev_returnColumnSizes[*returnSize - 1 - i];
	}
...
int main()
{
	...
	int **order = levelOrderBottom(root, &returnSize, &returnColumnSize);
	for (int i = 0; i < returnSize; i++)
	{
